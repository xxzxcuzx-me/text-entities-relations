...
describe("NerInterfaceService", () => {
  let service = new NerInterfaceService();

  beforeEach(() => {
    jest.resetAllMocks();
    jest.clearAllMocks();
    service = new NerInterfaceService();
  });
  ...
  it("should create one token and push it into array", () => {
    ...
    lastAnnotation = service["createToken"](
      wordCounter,
      sentence,
      annotation,
      token
  ...
  it("should create one sentence and push it into array", () => {
    const spy = jest.spyOn(service as any, "createToken");
    ...
    service["createSentence"](sentenceCounter, chunk, sentence);

    const newSentence = chunk.sentences.slice(-1)[0];
  ...
  it("should create one chunk and push it into array", () => {
    const spy = jest.spyOn(service as any, "createSentence");
    ...
    service["createChunk"](chunkCounter, chunk);

    const newChunk = service["result"].slice(-1)[0];
  ...
  it("should create chunkList of one chunk", () => {
    const spy = jest.spyOn(service as any, "createChunk");
    const NERData =
    ...
    service["createChunkList"](NERData);

    const chunkList = service["result"];
  ...
  it("should try to hit proper APIUrls.RESULT URL", async () => {
    ...
    const spyChunkList = jest.spyOn(service as any, "createChunkList");
    spyChunkList.mockReturnValue(null);
    const spyAxios = jest.spyOn(axios, "get");
    spyAxios.mockResolvedValue(NERData);
    const resultHandle = "/test";
    service["resultHandle"] = resultHandle;
  ...
  it("should try to hit proper APIUrls.RESULT URL and miss", async () => {
    const spyError = jest.spyOn(service as any, "handleError");
    spyError.mockReturnValue(null);
    const spyAxios = jest.spyOn(axios, "get");
    spyAxios.mockRejectedValue(new Error("test"));
    service["processResult"]().then(
      (resolve) => {
  ...
  it("should try to hit proper APIUrls.STATUS URL", async () => {
    ...
    const spyOnProcessResult = jest.spyOn(service as any, "processResult");
    spyOnProcessResult.mockResolvedValue(null);
    const spyOnProgress = jest.spyOn(service["_onProgress"] as any, "dispatch");
    spyOnProgress.mockReturnValue(null);
    const spyOnSuccess = jest.spyOn(service["_onSuccess"] as any, "dispatch");
    spyOnSuccess.mockReturnValue(null);
    const spyAxios = jest.spyOn(axios, "get");
    spyAxios
    ...
    service["taskHandle"] = taskHandle;
    service["interval"] = 10;
    await service["observeTask"]();
    expect(spyOnProgress).toHaveBeenCalledTimes(3);
    expect(spyOnProgress.mock.calls[0][0]).toBe(0.3333333333333333);
    expect(spyOnProgress.mock.calls[2][0]).toBe(1);
    expect(spyOnSuccess).toHaveBeenCalledTimes(1);
  ...
  it("should hit APIUrls.START URL", async () => {
    const NERData = {
    ...
    const properData = {
      lpmn: "filezip(" + service["fileHandle"] + ")|" + service["LPMN"],
      user: service["user"],
    };
    const spyObserveTask = jest.spyOn(service as any, "observeTask");
  ...
  it("should hit APIUrls.UPLOAD URL", async () => {
    ...
    const spyStartTask = jest.spyOn(service as any, "startTask");
    spyStartTask.mockResolvedValue(null);
    const spyAxios = jest.spyOn(axios, "post");
    spyAxios.mockResolvedValue(NERData);

    await service["processFile"](file);
    expect(spyStartTask).toHaveBeenCalledTimes(1);
    expect(spyAxios).toHaveBeenCalledTimes(1);
    expect(spyAxios.mock.calls[0][0]).toBe(
